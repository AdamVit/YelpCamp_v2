...
router.get('/users/:id', async function(req, res) {
	try {
		...
		Campground.find().where("author.id").equals(user._id).exec(function(err, campgrounds){
			var page;
			if(!req.isAuthenticated() || !(user.id == String(req.user._id))){
				return res.render('users/show', { user, campgrounds, page});
			}
			page = 'user';
			res.render('users/show', { user, campgrounds, page});
		});
	} catch(err) {
...
router.get('/unfollow/:id', middleware.isLoggedIn, async function(req, res) {
	try {
		...
		let self = await User.findById(req.user.id);
		if(!self.following.includes(user.id)){
			req.flash('error', "You do not follow this user yet!");
			return res.redirect('back');
		};
		user.followers.pull(req.user._id);
		await user.save();
		self.following.pull(user.id);
		await self.save();
